 ...
 * <li>v1.2, 111119
 * <ul>
 * <li>Converted ptInRect() calls to Rect.contains() calls.
 * <li>Fixed a bug when disabling some of the methods, wherein the method frame (hilight) was not properly assigned and drawn.
 * </ul>
 * <li>v1.1, 090408
 * <ul>
...
public class UberColorPickerDialog extends Dialog {
    ...
    private static class ColorPickerView extends View {
            ...
            for (int i = 0; i < NUM_ENABLED_METHODS; i++) {
                ...
                switch (mMethodSelectRectMap[i]) {
                    ...
                    case METHOD_HV_S_PALETTE: {
                        ...
                        if (mMethod == METHOD_HV_S_PALETTE)
                            hilightMethodSelectorOval(canvas);
                    }
                    break;
                    case METHOD_SV_H_PALETTE: {
                        ...
                        if (mMethod == METHOD_SV_H_PALETTE)
                    ...
                    case METHOD_UV_Y_PALETTE: {
                        ...
                        if (mMethod == METHOD_UV_Y_PALETTE)
                            hilightMethodSelectorRect(canvas);
                    }
                    break;
         ...
         */
        private void initUI() {
            while (mMethod < ENABLED_METHODS.length - 1 && !ENABLED_METHODS[mMethod])
                ++mMethod;
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            boolean inSwatchOld = mOldSwatchRect.contains(round(x), round(y));
            boolean inSwatchNew = mNewSwatchRect.contains(round(x), round(y));
            ...
            for (int i = 0; i < NUM_ENABLED_METHODS; i++) {
                if (mMethodSelectorRects[i].contains(round(x), round(y))) {
            ...
            boolean inSquarePalette = mPaletteRect.contains(round(x), round(y));

            //Is the event in a vertical slider to the right of the main 2D palette
            boolean inVerSlider = mVerSliderRect.contains(round(x), round(y));

            //Is the event in a horizontal slider within the main "palette's" region
            boolean inFirstHorSlider = mHorSliderRects[0].contains(round(x), round(y));
            boolean inSecondHorSlider = mHorSliderRects[1].contains(round(x), round(y));
            boolean inThirdHorSlider = mHorSliderRects[2].contains(round(x), round(y));
